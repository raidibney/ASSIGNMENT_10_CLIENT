...
const UserDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchDashboardPayloads = async () => {
      ...
      try {
        ...
        // 2. Map data fetching streams parallel arrays using live user email identity values
  ...
  // Fallback map safely checking database responses against existing state memory schemas
  const activeUserImage = profileData?.photo || session?.user?.image || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=150";
  const activeUserName = profileData?.name || session?.user?.name || "Active Passenger Member";
  const activeUserEmail = profileData?.email || session?.user?.email || "N/A";
  const activeUserRole = profileData?.role || session?.user?.role || "user";
          ...
          <div className="w-full p-8 rounded-3xl border border-base-content/10 bg-base-200/20 backdrop-blur-xl flex flex-col md:flex-row items-center md:items-start gap-8 animate-in fade-in slide-in-from-bottom-2 duration-200">
            ...
            <div className="flex-1 w-full text-center md:text-left">
              <span className="px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold uppercase tracking-wider bg-amber-500/10 border border-amber-500/20 text-amber-500">
                System tier: {activeUserRole}
              </span>
              <h2 className="text-3xl font-black tracking-tight mt-3 mb-1 text-base-content">{activeUserName}</h2>
              <p className="text-sm font-medium text-base-content/60 mb-6">{activeUserEmail}</p>
              ...
              <div className="grid sm:grid-cols-2 gap-4 max-w-xl text-left">
                <div className="p-4 rounded-2xl border border-base-content/5 bg-base-300/30">
                  ...
                  <p className="text-xs font-mono font-bold truncate text-base-content/80">
                    {profileData?._id || "Syncing Data Link..."}
                  </p>
                </div>
                <div className="p-4 rounded-2xl border border-base-content/5 bg-base-300/30">
                  ...
                  <p className="text-xs font-mono font-bold text-base-content/80">
                    {profileData?.lastActive ? new Date(profileData.lastActive).toLocaleDateString() : "Online Now"}
                  </p>
                </div>
              </div>
            </div>
